...
The release workflow builds artifacts in the same run because releases created with GitHub's default token do not trigger a second release workflow. electron-builder creates files locally, then `gh release upload` attaches them to the GitHub Release. macOS artifacts are built as an architecture matrix so Intel (`macos-15-intel`) and Apple Silicon packages are produced on native runners.
