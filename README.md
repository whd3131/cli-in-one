...
- Node.js `>=22`
- npm
- `codex`, `copilot`, `droid`, `claude`, or `agent` available in your `PATH` if you want project sessions to auto-start those CLIs
...
- Saved Codex quick presets, CMD command presets, and Quick Send prompts are written to `.cli-in-one/codex-quick-profiles.json`, `.cli-in-one/command-presets.json`, and `.cli-in-one/quick-prompts.json`.
...
- Node.js `>=22`
- npm
- 如果希望项目会话自动启动对应 CLI，需要保证 `codex`、`copilot`、`droid`、`claude` 或 `agent` 已在 `PATH` 中可用
...
- Codex 快捷配置方案、CMD 命令预置与快捷发送常用 prompt 会保存到 `.cli-in-one/codex-quick-profiles.json`、`.cli-in-one/command-presets.json` 和 `.cli-in-one/quick-prompts.json`。
